...
export default {
  ...
  methods: {
    getOpenId() {
      return new Promise((resolve) => {
        ...
        if (lsGet('openId') && lsGet('unionId')) {
          this.$store.commit('setOpenId', { openid: lsGet('openId'), unionid: lsGet('unionId') })
          resolve()
          return
        }
        const that = this
          ...
          success(data) {
            if (data.errMsg === 'login:ok') {
              ...
              wxGetOpenId({
                ...
                .then((res) => {
                  that.$store.commit('setOpenId', { openid: res.openId, unionid: res.unionId })
    ...
    async setInviteAndQuDaoCode(options) {
      ...
      // 将小程序启动options保存埋点
      if (options) {
        const data = { pageSession: Math.random().toString(16).substring(2) + new Date().getTime(), activityName: '小程序启动参数', activityContent: JSON.stringify(options) }
      ...
      if (options.path.includes('paradise')) {
        this.$store.commit('setHihiPageStart', true)
      }
