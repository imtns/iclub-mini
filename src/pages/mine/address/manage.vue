<template>
  <view class="address-manage">
    <uni-transition :show="!loading">
      ...
      <x-footer>
        <x-btn radius="32rpx" background="#366bd5" :disabled="submitting" @click="submitFn">保存</x-btn>
      </x-footer>
    </uni-transition>
    <x-toast ref="toast" />
  </view>
...
export default {
  ...
  onLoad(options) {
    ...
    wx.setNavigationBarTitle({
      title: this.objectCode ? '编辑地址' : '新增地址'
    })
    if (this.objectCode) {
      this.getDetail()
    }
  ...
  methods: {
    ...
    // 获取详情
    getDetail() {
      iclubApiUserReceiveAddressGet({ userCode: this.userCode, objectCode: this.objectCode })
        .then((res) => {
          const data = res || {}
          const info = {
