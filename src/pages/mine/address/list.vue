<template>
  <x-content :loading="loading">
    <view class="address-list" @tap.native.stop="$elementTracker">
      ...
      <scroll-view
        v-if="addressList && addressList.length && !loading"
        :style="{ height: `calc(100vh - ${addressList.length ? topHeight : 0}rpx - ${footerHeight}px)` }"
        class="list-wrap"
      ...
      <x-footer ref="footer">
        <x-btn class="submitBtn" background="#366bd5" :disabled="isManage" radius="32rpx" @click="addAddress">
...
export default {
  ...
  methods: {
    ...
    async getFooterHeight() {
      console.log('1')
      const footerHeight = await this.$refs.footer.getFooterHeight()
      console.log('🚀 ~ getFooterHeight ~ height:', footerHeight)
