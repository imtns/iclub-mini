    ...
    <view v-else>
      ...
      <view class="record-panel">
        ...
        >
          ...
          <!-- 加载更多（仅真实数据展示时显示） -->
          <uni-load-more v-if="recordList.length > 0" :status="loadMoreStatus" />
        </scroll-view>
...
.container {
  ...
  min-height: 100vh;
  background: linear-gradient(180deg, #d6eeff 0%, #f0f8ff 20%, #fff 100%);
...
.star-info-card {
  ...
  border-radius: 41rpx;
  background: linear-gradient(180deg, #008dff 0%, #0086f4 100%);
  box-shadow: 0 16rpx 40rpx rgba(0, 118, 255, 0.35);
}
