<template>
  <view class="img-container" :class="getID" @click="$emit('click')">
    ...
    <view v-else :style="{ 'border-radius': radius, position: 'relative', width: style.width, height: style.height, overflow: 'hidden' }">
      <view class="color-box" :style="{ background: style.background, width: style.width, height: style.height, borderRadius: radius }" />
      <uni-transition :show="show" mode="fade" :duration="300">
...
export default {
  ...
  computed: {
    ...
    finalSrc() {
      if (!this.src || this.src?.includes('gif')) return this.src
      let format = 'png'
