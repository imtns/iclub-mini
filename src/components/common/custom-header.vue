<template>
  <view>
    <!-- 自定义顶部栏 -->
    <view class="nav-bar" :class="{ transition }" :style="{ height: navBarHeight + 'px', backgroundColor: transparent ? 'transparent' : iconConfig[theme].bgColor }">
      <view class="halfPage">
    ...
    <x-toast ref="toast" />
    <uni-popup ref="noticePop" :mask-click="false" type="center" :z-index="99">
      <div style="position: relative">
        <div class="content-body">
          <div class="notice-content">
            ...
            <div v-if="systemUpdate && (systemUpdate.includes('账号异常') || systemUpdate.includes('注销'))" class="button" @click="go({ url: '/pages/mine/join-group/index?type=3' })">联系客服</div>
          </div>
        </div>
      </div>
    </uni-popup>
...
export default {
  ...
  computed: {
    ...mapState({
      systemUpdate: (state) => state.config.systemUpdate,
  ...
  watch: {
    systemUpdate: {
      handler(value) {
        ...
        const isCustomerPage = 'join-group/index'
        let pages = getCurrentPages()
        let currentPage = pages.pop()
        console.log('页面路径：', currentPage.route)
        if (currentPage.route.includes(isCustomerPage)) {
          return
        }
        if (value && this.systemUpdatePop) {
          setTimeout(() => {
            // 系统升级弹窗，以下手机白名单不弹窗
  ...
  mounted() {
    ...
    this.$store.commit('setNavBarHeight', this.navBarHeight)
