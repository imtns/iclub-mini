...
export default {
  ...
  methods: {
    async changeItem(item) {
      if (item.index === 3 && this.commonConfig.isShenHe) {
        // await subscribeMessage(mallSubscribeIds)
        this.report('首页tab跳转会员中心')
        if (!this.isLogin) {
          ls('returnUrl', '/pages-vipmall/index?registerPage=自主进入')
          this.goLogin()
          return
        }

        wx.navigateTo({
          url: '/pages-vipmall/index?registerPage=自主进入'
        })
      } else {
